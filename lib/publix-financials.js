...
function initializeQuarters(raw) {
  ...
  const qoqRev = computeQoQGrowth(withStores, 'revenue');
  const yoyRev = computeYoYGrowth(withStores, 'revenue');
  const yoyNI = computeYoYGrowth(withStores, 'netIncome');
  const qoqNI = computeQoQGrowth(withStores, 'netIncome');

  return withStores.map((q, i) => ({
    ...q,
