 ...
 *
 * Parses the date string manually to avoid the timezone bug where
 * new Date('YYYY-MM-DD') is UTC midnight but getMonth()/getFullYear()
 * return local-time values, shifting dates back a day in US timezones.
 */
export function endOfQuarter(dateStr) {
  const month = parseInt(dateStr.substring(5, 7), 10);
  const year  = parseInt(dateStr.substring(0, 4), 10);
  if (month === 1)  return new Date(year, 2, 31);
  if (month === 4)  return new Date(year, 5, 30);
  if (month === 7)  return new Date(year, 8, 30);
