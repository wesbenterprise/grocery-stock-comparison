...
/**
 * Helper: convert a PUBLIX_RAW date to end-of-PREVIOUS-quarter date.
 * The price announced on the effective date represents the value at the
 * end of the quarter before it:
 *   Jan 1 → Dec 31 prev year | Apr 1 → Mar 31 | Jul 1 → Jun 30 | Oct 1 → Sep 30
 *
 * Dates pinned to noon ET (17:00 UTC) so the calendar date is stable
 * regardless of the viewer's timezone. US markets operate on Eastern Time.
 */
const NOON_ET_UTC = 17;
export function endOfQuarter(dateStr) {
  const month = parseInt(dateStr.substring(5, 7), 10);
  const year  = parseInt(dateStr.substring(0, 4), 10);
  if (month === 1)  return new Date(Date.UTC(year - 1, 11, 31, NOON_ET_UTC));
  if (month === 4)  return new Date(Date.UTC(year, 2, 31, NOON_ET_UTC));
  if (month === 7)  return new Date(Date.UTC(year, 5, 30, NOON_ET_UTC));
  return new Date(Date.UTC(year, 8, 30, NOON_ET_UTC));
}

/**
 * Quarter label: the price represents end of the PREVIOUS quarter.
 * '2013-01-01' → 'Q4 2012', '2013-04-01' → 'Q1 2013', etc.
 */
