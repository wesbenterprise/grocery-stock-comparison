...
/**
 * Helper: convert a PUBLIX_RAW quarter-start date to end-of-quarter date.
 * Jan 1 (Q1) → Mar 31 | Apr 1 (Q2) → Jun 30 | Jul 1 (Q3) → Sep 30 | Oct 1 (Q4) → Dec 31
 *
 * The raw data date represents the quarter the price applies to; we plot
 * at the end of that quarter. E.g. '2025-10-01' ($19.65) → Dec 31, 2025.
 *
...
export function endOfQuarter(dateStr) {
  ...
  if (month === 1)  return new Date(Date.UTC(year, 2, 31, NOON_ET_UTC));
  if (month === 4)  return new Date(Date.UTC(year, 5, 30, NOON_ET_UTC));
  if (month === 7)  return new Date(Date.UTC(year, 8, 30, NOON_ET_UTC));
  return new Date(Date.UTC(year, 11, 31, NOON_ET_UTC));
}
