...
// Stocks in our basket (with S&P 500)
const BASKET = [
...
// URL-encode symbol for Yahoo Finance (^GSPC → %5EGSPC)
...
async function getQuarterlyPrice(symbol, targetDate) {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeSymbol(symbol)}?interval=1d&period1=${p1}&period2=${p2}`;
...
export async function GET() {
  try {
    ...
    const currentResults = await Promise.all(
      BASKET.map(async (s) => {
        const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeSymbol(s.symbol)}?interval=1d&range=5d`;
        try {
    ...
    const dailyData = await Promise.all(
      BASKET.map(async (s) => {
        const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeSymbol(s.symbol)}?interval=1d&period1=${q1p1}&period2=${q1p2}`;
        try {
          const res = await fetch(url, { headers: { Accept: 'application/json' } });
          const json = await res.json();
          const result = json.chart?.result?.[0];
