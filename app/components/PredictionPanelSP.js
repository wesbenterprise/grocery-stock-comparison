...
const COLORS = {
  ...
  sp500: '#a78bfa',    // purple — S&P 500
};

// Color map for weight bars by symbol
const WEIGHT_COLORS = {
  KR: '#f472b6',
  WMT: '#22d3ee',
  ADRNY: '#facc15',
...
export default function PredictionPanelSP() {
  ...
  if (loading) {
    return (
      <div className="prediction-panel loading">
        ...
        <p>Running regression on 6 stocks including S&P 500...</p>
      </div>
    );
  }
  ...
  return (
    <div className="prediction-panel">
      <div className="prediction-header">
        <h2>📈 Publix Price Predictor (with S&P 500)</h2>
        <p className="subtitle">
          Weighted basket of {data.weights.length} stocks including S&P 500 vs actual Publix (last {data.dataPoints} quarters)
        </p>
      </div>
            ...
            <div key={w.symbol} className="weight-bar">
              <div className="weight-info">
                <span className="weight-label">
                  {w.symbol === '^GSPC' && (
                    <span style={{ display: 'inline-block', width: '8px', height: '8px', borderRadius: '50%', backgroundColor: COLORS.sp500, marginRight: '6px', verticalAlign: 'middle' }} />
                  )}
                  {w.label}
                </span>
              ...
              <div className="weight-track">
                <div
                  ...
                  style={{
                    width: `${Math.min(w.weight * 100 * 2, 100)}%`,
                    ...(w.symbol === '^GSPC' ? { background: COLORS.sp500 } : {}),
                  }}
                />
              </div>
            </div>
        ...
        <div className="methodology-content">
          ...
          <div className="method-section">
            ...
            <p>
              ...
              Albertsons, and Weis Markets — plus the <strong style={{ color: COLORS.sp500 }}>S&P 500 index</strong> and
              ask: <em>what blend of these 6 assets best explains Publix's quarterly share price
              over the last 5 years?</em>
            </p>
            <p>
              First, every stock and the S&P 500 (including Publix) are <strong>normalized to an index</strong> starting
              at 1.0 in Q1 2021. This removes the dollar-price distortion (Walmart at $90 vs Publix
              at $12 vs S&P at ~4,000) and focuses purely on <em>how each has grown</em> over time.
            </p>
            <p>
              Then we run <strong>constrained least-squares regression</strong> to find the weight for
              each asset that minimizes the squared error between the weighted basket and actual Publix.
            ...
            <p>
              Including the S&P 500 lets us see how much of Publix's price movement is explained
              by the <strong>broad market</strong> vs grocery-sector peers specifically. The <strong>R² of {(data.r2 * 100).toFixed(1)}%</strong> means this
              basket explains {(data.r2 * 100).toFixed(0)}% of Publix's price movement. The <strong>RMSE
              of ${data.rmse}</strong> is the average dollar error per quarter.
            </p>
          </div>
          ...
          <div className="method-section">
            ...
            <p>
              ...
              relationship between these stocks, the S&P 500, and Publix, where does the model say Publix should
              be right now?</em>
            </p>
            <p>
              It takes today's price for each basket asset (including S&P 500), normalizes it to the same Q1 2021 index,
          ...
          <div className="method-section">
            ...
            <p>
              ...
              with the weighted basket (including S&P 500) during Q1 2026, where would it be today?</em>
            </p>
            <p>
              Starting from the last known Publix price (${lastActual.toFixed(2)} as of Q4 2025),
              we fetch <strong>daily closing prices</strong> for each basket stock and the S&P 500 from January 1, 2026
              to today. For each trading day, we compute each asset's return since Q1 open, weight
            ...
            <p>
              ...
              cumulative gap. If a stock is up 5% this quarter and has 37% weight, it contributes
            ...
            <p>
              ...
              much</em> since last quarter, based on what peers and the S&P 500 did this quarter." When the actual
